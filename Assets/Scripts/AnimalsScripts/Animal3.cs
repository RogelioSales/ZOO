...
public class Animal3 : MonoBehaviour
{
    ...
    IEnumerator Hungry()
    {
        ...
        MedicalIcon.SetActive(false);
        Debug.Log("Huungry");
        if (Input.GetKeyDown(KeyCode.Alpha5) && DataHolding.FoodGained >= amountofFoodloss)
        {
    ...
    IEnumerator Illness()
    {
        ...
        Debug.Log("Sick");
        if (Input.GetKeyDown(KeyCode.Alpha6) && DataHolding.MedicalGained >= amountofMedicalLoss)
        {
            DataHolding.AudienceCount += amountofAudienceGained;
